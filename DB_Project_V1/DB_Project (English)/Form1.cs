namespace DB_project
{
    public partial class Form1 : Form
    {
        ...
        {
            if (MessageBox.Show("Do you want to Delete All data of Table " + comboBox1.Text + "?", "Notification", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
        ...
        {
            if (MessageBox.Show("Do you want to delete all the selected record(s)?", "Notification", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                int index = 0;
                String position = DB.getColumnDataByColumnName("IndexColumns", "ordinal_position", comboBox1.Text);
                foreach (string i in position.Split("|"))
